...
//			idx[j] = narrow_bucket_ds->idx[data_num[j]];
			idx[j] = j;
			score[j] = expanded_priority(expanded_bucket_ds->sk[narrow_bucket_ds->idx[data_num[j]]], query_expanded_sketch);
		}
...
//		#if NUM_THREADS == 1
		...
		int k = num_candidates_2nd;
//		#else
//		int k = quick_select_k_para_work(idx, score, num_candidates_1st, num_candidates_2nd, wp);
...
//		int k = quick_select_k_para(idx, score, num_candidates_1st, num_candidates_2nd, NUM_THREADS);
//		for(int j = 0; j < k; j++) {
//			data_num[k++] = data_num_temp[idx[j]];
//		}
